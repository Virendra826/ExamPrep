...
4. **Key-Based Hybrid Reconciliation**: When `EXTRACTION_ENGINE=hybrid`, if a Gemini extraction encounters critical errors or low confidence, the pipeline cross-validates against the deterministic local parser by matching unique `questionNumber` keys (e.g. Q1 with Q1, Q3 with Q3) rather than fragile positional array indices. If local extraction achieved higher confidence, options and answers are merged while preserving visual diagram URLs. If the local parser detected questions that Gemini missed, they are safely preserved and surfaced as `LOCAL_ONLY_RECOVERED` candidates with audit notes.
...
7. **Traceability & Diagnostics**: Persisted questions created from ingestion batches retain a foreign key reference (`ingestion_batch_id`) pointing to their source `IngestionBatch`. Every extraction run emits structured diagnostic logs (engine, counts, confidence breakdown, reconciliation status) without leaking PDF content or API keys.
